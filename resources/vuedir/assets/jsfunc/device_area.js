import myjs from "./mjs_module";
let DevClsOBJ=null;
let DevVueOBJ=null;
let Timer=null;
let timercnt=40;

let deviceCLS = class DeviceArea {
   constructor(vOBJ, perm, tabstatus, pgname) {
       DevClsOBJ = this;
       DevVueOBJ = vOBJ;
       this.perm = perm;//
       this.pageLangs=[];
   ...
   checkTagContainer(){
    ...
    initPageLangs(lang){
       this.pageLangs=lang;
       //console.log(this.pageLangs);//del
    }
    getDevicesFromDB(){
        var flag=false;
        $.ajax({
    ...
    generateTag(){
       ...
       {
           var pgtxt_adaptative= myjs.data.getByAjaxtext(DevVueOBJ.$store.state.sitecontents, 'pgtxt_adaptative');
           var btntxt_add= myjs.data.getByAjaxtext(DevVueOBJ.$store.state.sitecontents, 'btntxt_add');
           var btntxt_change= myjs.data.getByAjaxtext(DevVueOBJ.$store.state.sitecontents, 'btntxt_change');
           ...
           if(this.nowEventState=="c")
            tags+=this.addDevicesOnTabWC();
           else
            tags+=this.addDevicesOnTabWA();
    ...
    addDevicesOnTabWA(){
        var cnt_devsplitlist = this.pageDevices.split(",");
        var item_tag='';
        var pgtxt_alldevices = myjs.data.getByAjaxtext(DevVueOBJ.$store.state.sitecontents, 'pgtxt_alldevices');
        if(this.pageDevices!="" && cnt_devsplitlist.length > 0)
        {
            ...
            else
            {
                for(var i=0;i<cnt_devsplitlist.length;i++)
                {
                    ...
                    if(cnt_devsplitlist.length > 1)
                        item_tag +='<span class="span_deviceitemDel_controller" data-cont="'+itemkey+'">&nbsp;&nbsp;|&nbsp;&nbsp; × </span>';
                    item_tag +='</span>';
                }
    ...
    devicePageDelete(key){
        var obj = document.getElementById('item_'+key);
        if(obj!=undefined && obj!=null){
            $('#item_'+key).remove();
            if(DevClsOBJ.selectedDevice == key)
                DevClsOBJ.selectedDevice = "";
            var alist=[];
            ...
            if(alist.length > 0)
                DevClsOBJ.pageDevices = alist.join(",");
            DevClsOBJ.generateTag();

            if(DevVueOBJ != undefined && DevVueOBJ != null)
                DevVueOBJ.PageInitByDevice_Area();
        }
    }
    deviceDelEvent(){
    ...
    deviceSelectEvent(){
        $('.page1_device_span').click(function () {
            ...
            if(DevClsOBJ.isShowAddBtn())
                $('.device_span_add_btn_area').addClass('active');
            else
                $('.device_span_add_btn_area').removeClass('active');

            if(DevClsOBJ.isShowChangeBtn())
                $('.device_span_change_btn_area').addClass('active');
            else
                $('.device_span_change_btn_area').removeClass('active');
   ...
   isShowChangeBtn(){
       ...
       {
           if(current_selected_word.length == 2)
               return true;
           else
           {
             if(splitpage[0].length == 1 && splitpage[1].length == 1)
                 return true;
           }
       }
    ...
    showAddDlg(){
        var pgtxt_save = myjs.data.getByAjaxtext(DevVueOBJ.$store.state.sitecontents, 'pgtxt_save');
        var pgtxt_adddevice = myjs.data.getByAjaxtext(DevVueOBJ.$store.state.sitecontents, 'pgtxt_adddevice');
        ...
        $('#'+this.name+'_modal_save_btn').click(function () {//process close event
    ...
    showChangeDlg(){
        ...
        var pgtxt_save = myjs.data.getByAjaxtext(DevVueOBJ.$store.state.sitecontents, 'pgtxt_save');
        var pgtxt_changedevice = myjs.data.getByAjaxtext(DevVueOBJ.$store.state.sitecontents, 'pgtxt_changedevice');
        ...
        if(this.nowEventState=="" && selectedword == "")
            selectedword = this.pageDevices;
        ...
        if(cnt_pgOfTab.length ==1){
            if(selectedword_s.length == 1)
               show_condion=0;//
                ...
                show_condion=1;//
            else if(selectedword_s.length == 3)
                show_condion=2; // 
        }
        ...
        $('#'+this.name+'_modal_save_btn').click(function () {//process close event
            ...
            {
                $('#'+DevClsOBJ.name+'_modal').modal('hide');
                if(DevClsOBJ.pageDevices.split(",").length==0)
                   myjs.data.showAlert('pgtxt_shouldonedevice');
            }
